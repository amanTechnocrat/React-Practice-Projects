...
class News extends Component {
    ...
    update = async () => {
        if (this.props.datasearch.length > 0) {
            let data = await fetch(`https://newsapi.org/v2/everything?q=${this.props.datasearch}&sortBy=publishedAt&apiKey=${this.state.apikey}&pageSize=6&language=en`)
            ...
            let data = await fetch(`https://newsapi.org/v2/top-headlines?category=${this.props.category}&apiKey=${this.state.apikey}&page=${this.state.page}&pageSize=${this.props.pageSize}&language=en`)
            let getdata = await data.json()
            // console.log(getdata);
            this.setState({
                articles: getdata.articles,
